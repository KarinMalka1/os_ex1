...
int uthread_terminate(int tid){
    ...
    // 2. טיפול בחוט הראשי (0)
    if (tid == 0) {
        for (auto const& [id, thread] : all_threads) {
            if (id != 0){
                delete[] 
                thread->stack; 
            }  
    ...
    // הסרה מהתור (READY) - דורש לעבור על התור ולמצוא אותו
    ...
    // אם החוט מחק את עצמו (מיועד לשלב הבא)
    if (running_thread->tid == tid) {
        // כאן בעתיד נצטרך לקרוא ל-Scheduler
    }
...
//context switch
void scheduler(){
    int sig = sigsetjmp(running_thread->env, 1);
    if ( sig == 1){
        return;
    }
